...
A) Simple DAG structure ith continuous outcome
...
imputation_methods <- c("mi", "fiml", "knn25", "knn5", "rf_flexible", "rf_less_flexible")
```
...
coverage_probs_low_MNAR <- data.frame(
...
avg_relative_biases_low_MNAR <- data.frame(
  missing_percentage = numeric(),
  imputation_method = character(),
...
for (missing_prob in missing_percentages) {
  ...
  coverage_probs_low_MNAR <- rbind(coverage_probs_low_MNAR, data.frame(
  ...
  avg_relative_biases_low_MNAR <- rbind(avg_relative_biases_low_MNAR, data.frame(
    missing_percentage = missing_prob,
...
print(avg_relative_biases_low_MNAR)
```
