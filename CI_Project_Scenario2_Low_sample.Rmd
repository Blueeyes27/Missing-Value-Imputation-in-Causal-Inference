...
estimate_ate <- function(data, method) {
  if (method == "mi") {
    ...
    library(missForest)
    imputed_data <- missForest(data, ntree = 100, nodesize = c(5, 5))$ximp
    medFit <- glm(M ~ A, family = gaussian, data = imputed_data)
    outcomeFit <- glm(Y ~ A + M + L1 + L2 + L3, family = gaussian, data = imputed_data)
    neMod <- mediation::mediate(model.m = medFit, model.y = outcomeFit,
                                 treat = "A", mediator = "M", data = data_check, robustSE = TRUE,
    ...
    library(missForest)
    imputed_data <- missForest(data, ntree = 5, nodesize = c(25, 5))$ximp
    medFit <- glm(M ~ A, family = gaussian, data = imputed_data)
    outcomeFit <- glm(Y ~ A + M + L1 + L2 + L3, family = gaussian, data = imputed_data)
    neMod <- mediation::mediate(model.m = medFit, model.y = outcomeFit,
                                 treat = "A", mediator = "M", data = data_check, robustSE = TRUE,
...
imputation_methods <- c("mi", "fiml", "knn25", "knn5
                        
                        ", "rf_flexible", "rf_less_flexible")

# Initialize a data frame to store the coverage probabilities
med_coverage_probs_low <- data.frame(
...
med_avg_relative_biases_low <- data.frame(
  missing_percentage = numeric(),
  imputation_method = character(),
...
for (missing_prob in missing_percentages) {
  ...
  med_coverage_probs_low <- rbind(med_coverage_probs_low, data.frame(
  ...
  med_avg_relative_biases_low <- rbind(med_avg_relative_biases_low, data.frame(
    missing_percentage = missing_prob,
...
print(med_coverage_probs_low)

#Print the average relative biases
cat("\nAverage Relative Biases:\n")
print(med_avg_relative_biases_low)
```

```{r}
write.csv(med_coverage_probs_low, file = "med_coverage_probs_low.csv", row.names = FALSE)
write.csv(med_avg_relative_biases_low, file = "med_avg_relative_biases_low.csv", row.names = FALSE)
```
