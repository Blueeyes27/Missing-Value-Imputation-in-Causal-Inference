...
# Read the CSV file
data <- read_csv("C:/Users/Kirtikanth/Downloads/Scenario2_med.csv")
...
create_plot <- function(df, sample_size, missing_type, measure) {
  ...
  plot <- ggplot(df, aes(x = missing_percentage, y = abs(get(paste0("average_relative_bias_", measure))) * 100, color = imputation_method)) +
    geom_line(size = 1) +
...
create_plot <- function(df, sample_size, missing_type, measure) {
  ...
  plot <- ggplot(df, aes(x = missing_percentage, y = get(paste0("coverage_probability_", measure)) * 100, color = imputation_method)) +
    geom_line(size = 1) +
    scale_color_manual(values = color_values) +
